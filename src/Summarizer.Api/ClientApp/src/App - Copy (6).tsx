...
function SummaryResult({ result }: { result: any }) {
  ...
  const bullets = extractBullets(analysis ?? result);
  const keywords = extractKeywords(analysis ?? result);
  ...
  return (
    <div className="mt-3 grid gap-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <ClipboardButton text={summaryText || ""} />
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {keywords.map((kw: string, i: number) => (
              <span
                key={i}
                className="inline-block bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-100 px-3 py-1 rounded-full text-sm"
              >
                {kw}
              </span>
            ))}
